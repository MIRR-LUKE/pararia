...
function modeLabel(mode: SessionConsoleMode, part: SessionConsoleLessonPart) {
  ...
  return part === "CHECK_OUT" ? "指導報告 / チェックアウト" : "指導報告 / チェックイン";
}
...
export function StudentSessionConsole({
  ...
  const [activeSessionId, setActiveSessionId] = useState<string | null>(ongoingLessonSession?.id ?? null);
  ...
  useEffect(() => {
    if (mode === "LESSON_REPORT" && ongoingLessonSession?.id) {
      setActiveSessionId(ongoingLessonSession.id);
    }
  }, [mode, ongoingLessonSession?.id]);
  ...
  const resolveTargetSessionId = useCallback(async () => {
    ...
    if (activeSessionId && mode === "LESSON_REPORT") return activeSessionId;
    return createSession();
  }, [activeSessionId, createSession, mode, ongoingLessonSession?.id]);
  ...
  const uploadAudioFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (body?.conversationId) {
          ...
          setMessage(
            lessonPart === "CHECK_IN"
              ? "チェックインを保存しました。授業後にチェックアウトを入れると指導報告が完成します。"
              : "チェックアウトを保存しました。生成が始まり次第、指導報告を確認できます。"
          );
          await onRefresh();
        }
      } catch (nextError: any) {
    ...
    [ensureLockForAudio, finalizeLock, lessonPart, mode, onRefresh, pollConversation, resolveTargetSessionId]
  );
  ...
  const idleDescription =
    mode === "INTERVIEW"
      ? "録音が終わると、自動で会話ログを整理して次回の話題まで更新します。"
      : lessonPart === "CHECK_OUT"
        ? "授業後の要点を残すと、次回の引き継ぎまで自動で整います。"
        : "授業前の一言を残しておくと、チェックアウトと合わせて 1 本の指導報告になります。";
    ...
    <div className={styles.console}>
      {showModePicker ? (
        <>
          ...
          {mode === "LESSON_REPORT" ? (
            <div className={styles.segmented} role="tablist" aria-label="指導報告の種類">
              <button
                type="button"
        ...
        <div className={styles.recorderArea}>
          ...
          <div className={styles.recorderMeta}>
            ...
            <div className={styles.statusLine}>
              {state === "recording"
                ? "話し終えたら終了してください。自動で保存して内容整理に入ります。"
                : state === "uploading" || state === "processing"
                  ? message
                  : message || idleDescription}
            </div>
            {ongoingLessonSession?.id && mode === "LESSON_REPORT" ? (
              <div className={styles.lessonMeta}>進行中の授業セッションに追記します。</div>
            ) : null}
          </div>
          ...
          {state === "processing" || state === "uploading" ? (
            <div className={styles.processingBox}>
              <strong>{state === "uploading" ? "保存中" : "生成中"}</strong>
              <p>
                {state === "uploading"
                  ? "音声を保存しています。完了すると自動で文字起こしを始めます。"
                  : "文字起こし、内容整理、おすすめの話題更新を順番に進めています。"}
              </p>
            </div>
          ) : null}
