...
export function buildWorkerEnv(input: {
  ...
  return {
    ...
    LLM_MODEL: process.env.LLM_MODEL?.trim() || "gpt-5.5",
    LLM_MODEL_FAST: process.env.LLM_MODEL_FAST?.trim() || process.env.LLM_MODEL?.trim() || "gpt-5.5",
    LLM_MODEL_REPORT: process.env.LLM_MODEL_REPORT?.trim() || process.env.LLM_MODEL?.trim() || "gpt-5.5",
