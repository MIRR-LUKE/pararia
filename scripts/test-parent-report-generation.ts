...
import { ReportArtifactValidationError } from "../lib/operational-log";
      ...
      logs: [
        {
          ...
          mode: "INTERVIEW",
          artifactJson: null,
          summaryMarkdown: "見た目だけはそれっぽい markdown があるが、artifactJson が無い。",
        },
      ],
    }),
  (error) => error instanceof ReportArtifactValidationError && /artifactJson が無効/.test(error.message)
);
