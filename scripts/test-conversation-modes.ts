...
function buildFinalizeResult(mode: MockMode) {
  if (mode === "LESSON_REPORT") {
    return {
      summaryMarkdown: [
        "## 会話で確認できた事実",
        "- 授業前には英語長文で根拠を拾い切れない不安が共有された。",
        "- 授業後には段落ごとの役割を押さえると正答率が上がった。",
        "",
        "## 指導の要点（講師が伝えた核）",
        "- 設問だけを追うのではなく本文の役割を先に捉える読み方を整理した。",
        "- 線引き位置を言葉で説明する練習まで含めて再現手順を固めた。",
        "",
        "## 次回までの方針",
        "- 英語長文は一日一題を目安に音読と根拠線引きを続ける。",
        "- 次回授業では迷った設問を一題確認し、判断順を再現できるかを見る。",
      ].join("\n"),
      timeline: [
        {
          title: "授業前の不安確認",
  ...
  return {
    summaryMarkdown: [
      "## 会話で確認できた事実",
      "- 模試の数学では最初の一手が出ずに止まる問題があった。",
      "- 英語は音読を続けた日に読み直しが減る実感が共有された。",
      "",
      "## 指導の要点（講師が伝えた核）",
      "- 数学は止まった瞬間を記録し、次回面談で思考の流れを一緒に見直す方針を伝えた。",
      "- 英語は短時間でも音読を継続し、読む感覚を落とさないことを整理した。",
      "",
      "## 次回までの方針",
      "- 数学は止まった一題の最初の一手をメモする。",
      "- 英語は音読を五日続け、次回面談で実行結果を確認する。",
    ].join("\n"),
    timeline: [
      {
        title: "模試後の不安整理",
...
async function main() {
  ...
  assert.match(lessonTranscript, /## セッション構成/);
  ...
  assert.ok(
    interviewResult.summaryMarkdown.includes("## 会話で確認できた事実") ||
      interviewResult.summaryMarkdown.includes("## 面談で確認した事実")
  );
