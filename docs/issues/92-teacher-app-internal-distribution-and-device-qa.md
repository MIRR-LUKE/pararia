...
- 録音中の一時停止 / 再開 / キャンセル二重確認、音声アップロード、Runpod stopped までを QA に含める
...
- `.github/workflows/android-internal-testing.yml` を追加し、手動実行で signed release APK artifact を作れるようにした
- 同じ workflow に、必要ならあとから Play Internal Testing upload へ広げられる分岐も残した
...
- `docs/teacher-app-internal-testing.md` を更新し、まずは Play Console なしで APK を直入れする最短手順を正本にした
...
- Android は workflow から signed APK を作って責任者端末へ渡せる
- Android は直入れか Play Internal Testing のどちらかで、実機 QA を実際に回している
- Android QA で `Runpod STT -> pod stopped -> 生徒確認 -> 完了` を記録として残している
