...
- lifecycle の暫定方針を `docs/teacher-app-lifecycle-policy.md` に切り出した
- Android / iOS ともに `createRecording` 成功後の recorder 起動失敗で orphan recording を残しにくくした
- Android / iOS ともに pending upload に `duration / attemptCount / lastAttemptAt` を残すようにした
- Android は foreground service を `FOREGROUND_SERVICE_TYPE_MICROPHONE` 明示で起動する形へ寄せた
- iOS は録音ファイルを `Application Support/TeacherRecordings` に置く形へ寄せた
