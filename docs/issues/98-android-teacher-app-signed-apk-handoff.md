...
- `Android Device Handoff` run `24949663628` は success
...
- local `apksigner verify --verbose --print-certs` も pass
- A142 / Android 16 で signed release APK の install / launch / login / main flow QA が完了
- `admin@demo.com` で device label `A142ReleaseQA` としてログインし、role `管理者` の READY 画面を確認
- 実スマホ音声 `【小輪瀬君】12-13-インタビュー_-大学受験数学の過去問戦略と共通テスト対策.mp3` を app から選択
- recordingId `cmofdv9z500093e81qshvwhbo` は STT job `cmofdveac0005t2ozb90m967d` が `DONE`
- manual select で `田中太郎` (`cmn784mqr0001ix8zumd351l4`) を選び、Session `cmofdzuwh000c3e81h92rzl6i` / ConversationLog `cmofdzva3000i3e818n6mlyzb` を生成
- DB で ConversationLog `DONE`、NextMeetingMemo `READY` を確認
- signed release で failure recovery も確認
  - recordingId `cmofeltzm000u3e8199yzy9yx`
  - upload phase で network off -> `pending_queued attemptCount=1`
  - READY 画面に `未送信 1` と `未送信を確認` が表示
  - `まとめて再送` -> `upload_success` -> `retry_upload_success attemptCount=2` -> `retry_result attempted=1 remaining=0`
  - `AWAITING_STUDENT_CONFIRMATION` 到達後、生徒なしで保存し `confirm_success` -> `done` -> READY
- retry STT job `cmofeo7kc0003vvf5wzbptcki` は `DONE`
- Runpod pod `scel1ckkaq7882` は QA 後 `desiredStatus=EXITED`
- QA 後に端末ネットワークと IME を復旧済み
...
- [x] `Android Device Handoff` を `upload_to_play=false` で実行する
- [x] signed release APK artifact を取得する
- [x] 実 Android 端末へ install / launch する
- [x] 校舎 QA 用に device / OS / app version / build number / base URL / recording ID を記録する
...
- なし。次回以降の校舎追加時は、この文書と `docs/teacher-app-internal-testing.md` の手順を再利用する。
...
2026-04-26 の A142 signed release QA で、上記はすべて満たした。追加 Issue 化が必要な失敗はなし。
