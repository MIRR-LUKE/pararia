...
| Name | 中身 |
| --- | --- |
| `FIREBASE_SERVICE_ACCOUNT_JSON` | サーバーから FCM push 通知を送るための Firebase service account JSON。未設定なら通知送信だけ安全にスキップ |
| `GCP_PROJECT_ID` | Vercel OIDC + Google Workload Identity Federation で FCM を送る場合の Firebase project id |
| `GCP_PROJECT_NUMBER` | Firebase project number |
| `GCP_SERVICE_ACCOUNT_EMAIL` | FCM 送信用 service account email |
| `GCP_WORKLOAD_IDENTITY_POOL_ID` | Vercel OIDC を信頼する Workload Identity Pool ID |
| `GCP_WORKLOAD_IDENTITY_POOL_PROVIDER_ID` | Vercel OIDC provider ID |

本番では service account key の長期保存より、Vercel OIDC + Google Workload Identity Federation を優先します。
Google 組織ポリシーで service account key 作成が禁止されている場合でも、上記 `GCP_*` を設定すれば短命 token で FCM HTTP v1 を呼び出せます。
...
| `PARARIA_FIREBASE_APPLICATION_ID` | Android app の Firebase Application ID。未設定なら端末側 push token 同期をスキップ |
| `PARARIA_FIREBASE_API_KEY` | Android app の Firebase API key |
| `PARARIA_FIREBASE_PROJECT_ID` | Firebase project id |
| `PARARIA_FIREBASE_SENDER_ID` | Firebase Cloud Messaging sender id |

`PARARIA_ANDROID_BASE_URL` は空なら workflow default の `https://pararia.vercel.app` が使われます。
指定する場合は `http://` か `https://` で始まる URL にしてください。
Firebase 系 Variable が揃っている端末ではログイン後に通知許可を求め、STT 完了時に「生徒確認してください」通知が届きます。揃っていない build では通知だけ無効で、録音・アップロード・polling はそのまま動きます。
