...
preflight は秘密値そのもの、password、keystore の中身を出力しません。ローカルで secret を環境変数に入れていない場合は、Secrets チェックが `FAIL` になるのが正常です。GitHub Actions でこの npm script を使う場合は、workflow 側で上の secret / variable を環境変数として渡してください。
...
- Secrets 登録そのものは GitHub repo owner が行う
...
2026-04-25 時点で、この端末は repo 側の静的 preflight 追加までは可能ですが、以下が無い場合は signed APK QA 完了までは進めません。

- `keytool` が無い場合:
  - JDK 17 以上を install し、`keytool` に PATH を通す
  - そのあと `npm run test:android-release-handoff-preflight` を再実行する
- `adb` が無い場合:
  - Android SDK Platform Tools を install し、`adb` に PATH を通す
  - 端末の USB debugging を有効化して `adb devices` で認識を確認する
