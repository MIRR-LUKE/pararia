...
function buildInterviewMarkdownStablePrefixLines(draftInputLabel: string) {
  return [
    ...
    "",
    "固定品質ゲート:",
    "- 生成前に `話題の有無 / 根拠の有無 / section の重複` をこの順番で確認する。",
    "- 生成前に、重要発話から `学習状況 / 課題 / 対策 / 進路 / 次回確認` の候補だけを内的に分類する。",
    "- 生成時は、候補がない section を膨らませず、短い欠落表示で閉じる。",
    "- 生成時は、同じ話題を `サマリー` と箇条書きに重ねる場合も、言い回しではなく役割を変える。",
    "- `サマリー` は流れ、`学習状況と課題分析` は現在地、`今後の対策・指導内容` は次の行動として分ける。",
    "- `志望校に関する検討事項` は進路・学校名・方式・面談予定が出たときだけ使う。",
    "- `次回のお勧め話題` は先生が次回そのまま聞ける確認文に寄せる。",
    "- 口語の勢いを残すより、管理者が後から判断できる短い記録にする。",
    "- 事実の強さが弱いものは断定せず、本文に入れる優先度を下げる。",
    "- ノイズ、相づち、録音開始の合図、質問ありますか系の定型句は本文に入れない。",
    "- 生徒名・講師名・日付・面談時間は基本情報だけで使い、本文の根拠にはしない。",
    "- 教材名、学校名、受験方式、学習行動、時間配分などの具体語が transcript にあるときは優先する。",
    "- 重要発話と全文が食い違うように見えるときは、全文を優先して控えめに書く。",
    "- 指導方針は `何をするか` まで書き、抽象的な励ましだけで終えない。",
    "- 次回確認は `できたか / どう変わったか / 何を続けるか` のどれかに落とす。",
    "- 出力は markdown 本文だけにし、内部分類や品質ゲートの説明は出さない。",
  ];
}
