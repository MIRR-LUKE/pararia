...
function quoteEvidenceLine(line: string) {
  const cleaned = dedupeKeepOrder([line])[0];
  if (!cleaned) return [];
  return [`- ${cleaned}`, `  根拠: ${cleaned}`];
}

function renderQuotedLines(lines: string[], fallback: string, limit = 4) {
  const picked = dedupeKeepOrder(lines).slice(0, limit);
  if (picked.length === 0) {
    return [`- ${fallback}`, `  根拠: ${fallback}`];
  }
  return picked.flatMap((line) => quoteEvidenceLine(line));
}

function sectionWithQuotedLines(title: string, lines: string[], fallback: string, limit = 4) {
  return [title, ...renderQuotedLines(lines, fallback, limit)];
}
...
export function buildInterviewDraftFallbackMarkdown(input: {
  ...
  return repairSummaryMarkdownFormatting(
    [
      ...
      "",
      "■ 1. サマリー",
      ...renderQuotedLines(lines.slice(0, 4), "今回の面談で確認できた内容は限定的だった。", 2),
      ...renderQuotedLines(lines.slice(4, 8), "次回までの確認事項は transcript から拾えた範囲に限る。", 2),
      "",
      "■ 2. ポジティブな話題",
      ...positiveLines.flatMap((line) => quoteEvidenceLine(line)),
      "",
      "■ 3. 改善・対策が必要な話題",
      ...issueLines.flatMap((line) => quoteEvidenceLine(line)),
      "",
      "■ 4. 保護者への共有ポイント",
      ...parentLines.flatMap((line) => quoteEvidenceLine(line)),
    ].join("\n")
  );
}
...
export function buildLessonDraftFallbackMarkdown(input: {
  ...
  return repairSummaryMarkdownFormatting(
    [
      ...
      "",
      "■ 1. 本日の指導サマリー（室長向け要約）",
      ...renderQuotedLines([...checkInLines.slice(0, 2), ...lines.slice(0, 2)], "授業内容の確認範囲は限定的だった。", 2),
      ...renderQuotedLines([...checkOutLines.slice(0, 2), ...lines.slice(2, 4)], "理解状況と次回への接続は transcript から拾えた範囲に限る。", 2),
      "",
      "■ 2. 課題と指導成果（Before → After）",
      "【授業前の理解状況】",
      ...renderQuotedLines(checkInLines.slice(0, 3), "授業前の確認内容は限定的だった。", 3),
      "【授業後の理解状況】",
      ...renderQuotedLines(checkOutLines.slice(0, 3), "授業後の確認内容は限定的だった。", 3),
      "",
      "■ 3. 学習方針と次回アクション（自学習の設計）",
      "生徒:",
      ...dedupeKeepOrder(lines.slice(10, 13)).flatMap((line) => quoteEvidenceLine(line)),
      "次回までの宿題:",
      ...dedupeKeepOrder(lines.slice(13, 16)).flatMap((line) => quoteEvidenceLine(line)),
      "次回の確認（テスト）事項:",
      ...dedupeKeepOrder(lines.slice(16, 19)).flatMap((line) => quoteEvidenceLine(line)),
      "",
      "■ 4. 室長・他講師への共有・連携事項",
      ...dedupeKeepOrder(lines.slice(19, 23)).flatMap((line) => quoteEvidenceLine(line)),
    ].join("\n")
  );
}
